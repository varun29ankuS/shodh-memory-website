...
const BLOG_CONTENT: Record<string, string[]> = {
  "causal-retrieval-ai-memory": [
    ...
    "",
    "A retrieval-augmented system embeds that question, searches its vector store for the nearest passages, and hands back whatever text is *semantically similar* to the words \"decide,\" \"Rust,\" and \"backend.\" Sometimes that works — if the decision and its reasoning happen to sit in one passage that looks like the question. Often it does not, because the *reason* for a decision is rarely phrased like the decision itself, and the chain that led to it is scattered across many memories laid down at different times.",
    "",
    "This is not a tuning problem. It is a structural one. Semantic similarity answers a specific question — *\"what looks like this?\"* — and it answers it well. But \"why did this happen?\" is a different question entirely, and no amount of better embeddings will turn a nearest-neighbor search into a causal explanation. Recent surveys of agent memory name this gap directly, listing hybrid retrievers that blend semantic similarity with temporal ordering and causal graph traversal as one of the field's *largely-unexplored* frontiers. This post is about why that gap exists and what it actually takes to close it.",
    "",
    "## Two different questions",
    "",
    "It helps to be precise about the distinction:",
    "",
    "- **\"What looks like this?\"** — a *similarity* query. Given a point, find the nearest points. This is what vector search does. It is associative, content-addressable, and order-free: it does not care when things happened or what led to what.",
    "- **\"What caused this?\"** — a *causal* query. Given an outcome, find the chain of antecedents that produced it: the decision behind the result, the incident behind the decision, the observation behind the incident. This is a *walk backward through a structure*, not a lookup of nearby points.",
    "",
    "These two questions need two different data structures. Similarity needs a metric space — a place where \"near\" is defined. Causality needs a *directed graph* — a place where \"X led to Y\" is an edge you can follow, in a direction. A vector store is a metric space. It has no edges and no direction. You cannot traverse a causal chain in a structure that has no chains in it.",
    "",
    ...
    "- **Similarity actively misleads.** The passages most *similar* to \"why did we choose Rust\" are often other discussions of Rust — not the reasoning that preceded the choice. Similarity pulls toward the topic and away from the cause.",
    ...
    "",
    "The honest summary: flat retrieval can *sometimes* surface a cause when it happens to be similar and nearby. It cannot *reliably reconstruct a causal chain*, because it has no representation of causality to reconstruct from.",
    "",
    ...
    "",
    "1. **Typed, directed edges.** Not a generic \"these two memories are related,\" but \"this memory *caused* that one\" — a relation with a direction. The arrow matters: \"the incident caused the decision\" is true; \"the decision caused the incident\" is false. Direction is the whole point of causality.",
    ...
    "3. **A way to score the walk.** Real histories are noisy; not every \"because\" is load-bearing. The walk has to be scored and bounded — favoring the strongest causal paths, decaying with distance, stopping before it floods — so it returns *the* origin, not every weakly-connected ancestor.",
    "",
  ...
  "internet-of-things-examples-memory": [
    ...
    "",
    "**With memory:** the robot remembers — spatially indexed — where it has been, what it observed, and why it acted. It recalls that the loading dock door near the north pump tends to be blocked at shift change, that it saw corrosion on a specific pipe last week, that a particular corridor has poor traction. Its perceptual recognizers adapt to *this* facility over its operational life. None of this needs the cloud, which matters because the robot is often disconnected and always on a control loop too fast for a network round-trip. This is the example where memory stops being a nicety and becomes a requirement.",
    "",
  ...
  "ai-model-pricing-guide-2026": [
    ...
    "",
    "**Important:** The Assistants API — threads, runs, file search — is deprecated as of August 2026. If you built memory on top of Assistants, you need a migration plan. See our [migration guide](/blog/openai-assistants-api-deprecated-alternative).",
    "",
    "---",
    "",
    "## Google — Gemini 2.5",
    ...
    "- Apache 2.0 licensed, 1089 tests, [research paper](https://doi.org/10.5281/zenodo.18668709)",
    "",
    "```bash",
    ...
    "- [Compare with other memory systems](/blog/shodh-memory-vs-mem0-vs-zep-vs-memgpt)",
  ],
  "types-of-memory-in-ai": [
    ...
    "",
    "Cowan (2001) refined the capacity estimate. Miller's (1956) famous \"7 plus or minus 2\" was measuring *chunks*, not items. When you control for chunking, the true capacity of the focus of attention is about 4 items. This has profound implications for AI: context windows are not working memory. A 200K token context window is more like a very large short-term buffer — the *active working set* at any moment is much smaller.",
    "",
    ...
    "",
    "Episodic memory stores specific events anchored in time and place. Endel Tulving (1972) identified it as a distinct system — it is not just facts, but the *experience* of having been there.",
    "",
    "```",
    "┌──────────────────────────────────────────────────┐",
    ...
    "",
    "Semantic memory stores general knowledge stripped of episodic context. You know that Paris is the capital of France, but you probably do not remember the specific moment you learned it. The fact has been *decontextualized* — extracted from its episodic origin and stored as pure knowledge.",
    "",
    "```",
    "┌──────────────────────────────────────────────────┐",
    ...
    "",
    "Procedural memory stores *how* to do things — motor skills, cognitive procedures, habitual behaviors. You do not consciously remember learning to type, but your fingers know where the keys are. This knowledge is implicit: it influences behavior without conscious recall.",
    "",
    "```",
    "┌──────────────────────────────────────────────────┐",
    ...
    "",
    "- **Focus of attention** = working memory tier (4-7 most recent/active items)",
    "- **Activated LTM** = session memory tier (current session context, rapidly accessible)",
    "- **Full LTM** = long-term tier (all consolidated memories, retrieved by spreading activation)",
    "",
    "---",
    "",
    ...
    "",
    "- **Tier 1 (L1 Working):** Single co-access creates a weak edge (+0.025 strength). Fades quickly without reinforcement.",
    "- **Tier 2 (L2 Episodic):** Repeated co-access within hours promotes the edge. Moderate strength, slower decay (multiplicative *0.90).",
    "- **Tier 3 (L3 Semantic):** Sustained co-access over days promotes to permanent. The edge represents established knowledge.",
    "",
    ...
    "",
    "Semantic memory in the brain is a distributed network of concepts connected by associations. \"Dog\" connects to \"animal,\" \"pet,\" \"bark,\" \"fur\" — and through those connections, to \"cat,\" \"veterinarian,\" \"leash.\" Retrieval is not lookup — it is *spreading activation* through a web of associations.",
    "",
    "```",
    "┌──────────────────────────────────────────────────────┐",
    ...
    "```",
    "",
    "This is fundamentally different from vector similarity search. Vectors find memories that *look like* the query. Spreading activation finds memories that are *connected to* the query through learned associations. Both are valuable. The combination is more powerful than either alone.",
    "",
    "**In shodh-memory:** The 5-layer retrieval pipeline combines vector search, knowledge graph traversal with spreading activation, temporal context, and recency — then fuses results using reciprocal rank fusion (RRF). Memories that score high on both vector similarity *and* graph connectivity rank highest. This is why shodh retrieves relevant context that pure vector databases miss.",
    "",
    "---",
    "",
    ...
    "",
    "Every serious AI system will need a memory architecture. Not a vector database. Not a chat history buffer. A *memory architecture* — with distinct types, encoding mechanisms, consolidation processes, and retrieval strategies.",
    "",
    "The context window is not memory. It is the computational workspace where memory gets *used*. Confusing the two is like confusing RAM with a hard drive — they serve fundamentally different purposes, and a system with only one will always be crippled.",
    "",
    ...
    "- [GitHub](https://github.com/varun29ankuS/shodh-memory)",
    "- [Documentation](https://www.shodh-memory.com/docs)",
    "- [Research Paper (DOI: 10.5281/zenodo.18668709)](https://doi.org/10.5281/zenodo.18668709)",
    "",
    "## References",
    "",
    ...
    "5. Tulving, E. (1972). Episodic and Semantic Memory. In *Organization of Memory*, Academic Press.",
  ...
  "what-your-ai-doesnt-know-it-doesnt-know": [
  ...
  "shodh-memory-vs-mem0-vs-zep-vs-memgpt": [
  ...
  "how-to-make-ai-remember-between-sessions": [
  ...
  "claude-code-cursor-memory-setup": [
  ...
  "robotics-needs-memory": [
    ...
    "",
    "Meanwhile, the hardware is evolving too. Intel's Loihi 2 and IBM's TrueNorth run spiking neural networks at under a watt — Hebbian learning and synaptic decay in silicon, not software. As neuromorphic chips move from research labs to production, the robots that already have cognitive memory will be the first to benefit.",
    "",
  ...
  "edge-ai-memory-raspberry-pi": [
    ...
    "## Conclusion",
    "",
    "Edge AI memory isn't a future promise—it's available now. The Pi proves that meaningful AI can run on meaningful hardware constraints. And the floor keeps dropping — neuromorphic chips like Intel's Loihi 2 already run cognitive workloads at under a watt. As that hardware matures, edge memory systems will only get faster and cheaper to operate.",
  ],
  ...
  "chatgpt-memory-full-unlimited-alternative": [
    ...
    "",
    "This is not a hypothetical. This is how [shodh-memory](https://github.com/varun29ankuS/shodh-memory) works.",
    "",
    "```",
    "┌───────────────────────────────────────────────┐",
  ...
  "openai-assistants-api-deprecated-alternative": [
    ...
    "```",
    "",
    "shodh-memory is a single Rust binary (~30MB). It runs 100% offline. No Docker, no cloud, no API keys. Apache 2.0 licensed. 1089 tests. Published on [crates.io](https://crates.io/crates/shodh-memory), [npm](https://www.npmjs.com/package/@shodh/memory-mcp), and [PyPI](https://pypi.org/project/shodh-memory/).",
    "",
  ...
  "mcp-memory-server-guide": [
    ...
    "",
    "The MCP tools above are *reactive* — the AI calls them when it decides to. But the best memory is **automatic**. You should not have to ask your agent to remember; it should happen as a side effect of working.",
    "",
    ...
    "",
    "MCP gives your agent memory access. But access alone is not enough. What matters is the *quality* of retrieval — whether the right memories surface at the right time.",
    "",
    ...
    "```",
    "",
    "This pipeline means your agent does not just find memories that *mention* the right keywords. It finds memories that are *semantically related*, *structurally connected* via the knowledge graph, *temporally relevant*, and *emotionally salient*. A memory stored three months ago about a critical production bug will surface when you discuss the same system — even if you use completely different words.",
    "",
    "---",
    "",
    ...
    "",
    "- [GitHub](https://github.com/varun29ankuS/shodh-memory)",
    "- [npm](https://www.npmjs.com/package/@shodh/memory-mcp)",
    "- [Documentation](https://www.shodh-memory.com/docs)",
    "- [Research Paper (DOI)](https://doi.org/10.5281/zenodo.18668709)",
  ],
  "best-ai-agent-frameworks-2026": [
    ...
    "- [Documentation](https://www.shodh-memory.com/docs)",
    "- [Research Paper (DOI)](https://doi.org/10.5281/zenodo.18668709)",
    "- [MCP Setup Guide](/blog/mcp-memory-server-guide)",
  ],
  "graph-databases-for-ai-memory": [
    ...
    "- [Documentation](https://www.shodh-memory.com/docs)",
    "- [npm](https://www.npmjs.com/package/@shodh/memory-mcp)",
    "- [Crates.io](https://crates.io/crates/shodh-memory)",
    "- [PyPI](https://pypi.org/project/shodh-memory/)",
    "- [DOI: 10.5281/zenodo.18668709](https://doi.org/10.5281/zenodo.18668709)",
  ],
  "cognitive-architectures-for-ai-agents-guide": [
    ...
    "",
    "A **cognitive architecture** is a blueprint for how an intelligent system perceives, reasons, remembers, and acts. It is not a model — it is the *scaffolding* on which models operate.",
    "",
  ...
  "vector-databases-how-they-work": [
    ...
    "",
    "A **vector** is a list of numbers. An **embedding** is a vector produced by a neural network that captures the *meaning* of its input. Texts with similar meanings produce vectors that are close together in vector space.",
    "",
    "```",
    "┌─────────────────────────────────────────────────────────────────────┐",
    ...
    "",
    "### 2. Strengthening",
    "When you retrieve a memory, it should become *easier* to retrieve next time. Vector databases treat retrieval as read-only. shodh-memory updates access counts, timestamps, and graph edge weights on every recall.",
    "",
    "### 3. Relationships",
